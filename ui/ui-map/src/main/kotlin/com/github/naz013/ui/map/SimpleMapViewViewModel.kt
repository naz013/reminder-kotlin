...
import com.github.naz013.ui.map.place.UiPlaceList
...
class SimpleMapViewViewModel(
  ...
  private val radiusRange = RadiusRangeState(mapPreferences.radius)
  private val radiusFormatter = DefaultRadiusFormatter(textProvider, mapPreferences.useMetric)
  ...
  fun onMapTypeSelected(type: Int) {
    if (mapParams.rememberMapStyle) mapPreferences.mapType = type
    _state.update {
      it.copy(
  ...
  fun onMapStyleSelected(style: Int) {
    if (mapParams.rememberMapStyle) mapPreferences.mapStyle = style
    _state.update {
      it.copy(
        selectedMapStyle = style,
        mapStyleOptions = mapStyle.getMapStyleOptions(style, mapPreferences.mapType),
        activePicker = null
      )
    }
  }
  ...
  fun onMarkerStyleSelected(style: Int) {
    if (style != _state.value.selectedMarkerStyle && mapPreferences.hapticsEnabled) {
      event.emit(MapEvent.HapticFeedback)
    }
    if (mapParams.rememberMarkerStyle) mapPreferences.markerStyle = style
  ...
  fun onRadiusChanged(value: Float) {
    ...
    if (radiusRange.radius != previousRadius && mapPreferences.hapticsEnabled) {
      event.emit(MapEvent.HapticFeedback)
    }
    if (mapParams.rememberMarkerRadius) mapPreferences.radius = radiusRange.radius
    _state.update {
      it.copy(
        radiusMeters = radiusRange.radius,
        radiusText = radiusFormatter.format(radiusRange.radius)
  ...
  fun onRecentPlaceSelected(place: UiPlaceList) {
    val style = if (buildInfo.isPro) place.markerStyle else _state.value.selectedMarkerStyle
  ...
  private fun loadInitialState(current: MapUiState): MapUiState {
    return current.copy(
      selectedMapType = mapPreferences.mapType,
      selectedMapStyle = mapPreferences.mapStyle,
      selectedMarkerStyle = if (buildInfo.isPro) {
      ...
      maxRadiusMeters = radiusRange.valueTo,
      radiusText = radiusFormatter.format(radiusRange.radius),
      mapStyleOptions = mapStyle.getMapStyleOptions(mapPreferences.mapStyle, mapPreferences.mapType),
      markerStyleSliderColors = mapStyle.colorsForSlider(),
      hapticFeedbackEnabled = mapPreferences.hapticsEnabled,
      isPro = buildInfo.isPro,
    )
  }
