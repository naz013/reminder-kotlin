...
private const val SELECTOR_CONTRAST_LUMINANCE_THRESHOLD = 0.5f
...
fun ColorSlider(
  ...
  val semanticsModifier = Modifier.colorSliderSemantics(
  ...
  Canvas(
    ...
    val geometry = ColorSwatchGeometry(
      itemWidth = size.width / colors.size,
      verticalInset = size.height * UnselectedItemVerticalInset,
      strokeWidthPx = SelectorStrokeWidth.toPx(),
    )
...
private suspend fun PointerInputScope.detectColorSliderDrag(
  colorCount: Int,
  ...
  fun selectAt(x: Float) {
    val itemWidth = size.width / colorCount.toFloat()
    val index = (x / itemWidth).toInt().coerceIn(0, colorCount - 1)
    if (hapticFeedbackEnabled && selectedIndex() != index) {
...
private fun Modifier.colorSliderSemantics(
...
/**
 * [selectorColor] can vanish against a swatch of that exact same color (e.g. a pure black/white
 * swatch with the usual theme-based black/white selectorColor), so the selected swatch's ring
 * falls back to a color contrasting with the swatch's own luminance in that case only - keeping
 * the same single-ring geometry everywhere else instead of risking a second, nested ring that
 * won't fit inside a narrow swatch on a strip with many colors.
 */
private data class ColorSwatchGeometry(
  val itemWidth: Float,
  val verticalInset: Float,
  val strokeWidthPx: Float,
)

private fun DrawScope.drawColorSwatch(
  index: Int,
  color: Color,
  selectedIndex: Int,
  selectorColor: Color,
  geometry: ColorSwatchGeometry,
) {
  val (itemWidth, verticalInset, strokeWidthPx) = geometry
  ...
  if (index == selectedIndex) {
    ...
    val ringColor = if (color == selectorColor) {
      if (color.luminance() > SELECTOR_CONTRAST_LUMINANCE_THRESHOLD) Color.Black else Color.White
    } else {
      selectorColor
    }
    drawRect(
