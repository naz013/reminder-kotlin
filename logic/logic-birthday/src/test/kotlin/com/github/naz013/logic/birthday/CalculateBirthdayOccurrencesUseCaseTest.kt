 ...
 */
class CalculateBirthdayOccurrencesUseCaseTest {
  private lateinit var birthdayPreferences: BirthdayPreferences
  ...
  @Test
  fun `invoke should calculate and save birthday occurrences with valid data`() =
    runTest {
      ...
      every { birthdayPreferences.numberOfBirthdayOccurrences } returns numberOfOccurrences
  ...
  @Test
  fun `invoke should handle birthday not found gracefully`() =
    runTest {
      // Arrange
  ...
  @Test
  fun `invoke should handle invalid birthday date gracefully`() =
    runTest {
      // Arrange
  ...
  @Test
  fun `invoke should use current time when birthday time is null`() =
    runTest {
      // Arrange
  ...
  @Test
  fun `invoke should delete existing occurrences before creating new ones`() =
    runTest {
      // Arrange
      ...
      every { birthdayPreferences.numberOfBirthdayOccurrences } returns 2
      coEvery { birthdayRepository.getById(birthdayId) } returns birthday
      every { dateTimeManager.parseBirthdayDate(birthdayDate) } returns parsedDate
      every { dateTimeManager.getBirthdayLocalTime() } returns birthdayTime
      every { dateTimeManager.getCurrentDate() } returns LocalDate.of(currentYear, 1, 1)
  ...
  @Test
  fun `invoke should generate correct number of occurrences based on preferences`() =
    runTest {
      // Arrange
  ...
  @Test
  fun `invoke should start occurrences from previous year`() =
    runTest {
      // Arrange
  ...
  @Test
  fun `invoke should handle leap year birthdays correctly`() =
    runTest {
  ...
  @Test
  fun `invoke should preserve birthday UUID in all occurrences`() =
    runTest {
      // Arrange
      ...
      every { birthdayPreferences.numberOfBirthdayOccurrences } returns 3
      coEvery { birthdayRepository.getById(birthdayId) } returns birthday
      every { dateTimeManager.parseBirthdayDate(birthdayDate) } returns parsedDate
      every { dateTimeManager.getBirthdayLocalTime() } returns birthdayTime
      every { dateTimeManager.getCurrentDate() } returns LocalDate.of(currentYear, 1, 1)
