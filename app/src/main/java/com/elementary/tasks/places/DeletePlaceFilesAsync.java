...
public class DeletePlaceFilesAsync extends AsyncTask<String, Void, Void> {

    private Context mContext;

    public DeletePlaceFilesAsync(Context context) {
    ...
    protected Void doInBackground(String... params) {
        for (String uid : params) {
            String exportFileName = uid + FileConfig.FILE_NAME_PLACE;
            ...
            if (isConnected) {
                new Dropbox(mContext).deletePlace(exportFileName);
                Google google = Google.getInstance(mContext);
                if (google != null && google.getDrive() != null) {
                    try {
                        google.getDrive().deletePlaceFileByName(exportFileName);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
