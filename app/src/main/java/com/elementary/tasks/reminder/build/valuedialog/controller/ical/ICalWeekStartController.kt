...
class ICalWeekStartController(
  ...
  override fun getIndex(t: DayValue?): Int = t?.day?.let { Day.entries.indexOf(it) } ?: 0

  override fun getSelectionItems(): List<String> = Day.entries.map { DayValue(it) }.map { paramToTextAdapter.getDayFullText(it) }
