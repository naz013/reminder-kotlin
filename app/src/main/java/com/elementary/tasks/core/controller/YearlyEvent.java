...
class YearlyEvent extends RepeatableEventManager {

    YearlyEvent(Reminder reminder, Context context) {
    ...
    public long calculateTime(boolean isNew) {
        return TimeCount.getInstance(getContext()).getNextYearDayTime(getReminder());
    }
}