...
class DeepLinkDataParser {
  ...
  fun readDeepLinkData(intent: Intent): DeepLinkData? {
    val key = findKey(intent) ?: return null
    val clazz = deepLinkDataMap[key] ?: return null
    return readParcelable(intent, key, clazz)
  }

  fun readDeepLinkData(bundle: Bundle): DeepLinkData? {
    val key = findKey(bundle) ?: return null
    val clazz = deepLinkDataMap[key] ?: return null
    return readParcelable(bundle, key, clazz)
  }

  private fun findKey(intent: Intent): String? = deepLinkDataMap.keys.firstOrNull { intent.hasExtra(it) }

  private fun findKey(bundle: Bundle): String? = deepLinkDataMap.keys.firstOrNull { bundle.containsKey(it) }
