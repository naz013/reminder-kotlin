...
class NotesViewModelTest : BaseTest() {
  ...
  override fun setUp() {
    super.setUp()
    every { prefs.isNotesGridEnabled } returns false
    every { prefs.noteOrder } returns NoteSortProcessor.DATE_ZA
  ...
  fun `onSortOrderSelected persists the sort order and updates state`() =
    runTest {
      ...
      verify { prefs.noteOrder = NoteSortProcessor.TEXT_AZ }
  ...
  fun `onGridToggleClick flips the grid flag and persists it`() =
    runTest {
      ...
      verify { prefs.isNotesGridEnabled = true }
  ...
  fun `showNoteInNotification shows the notification for the note`() =
    runTest {
      ...
      verify(exactly = 1) { notifier.showNoteNotification(uiNotification) }
    }
  ...
  fun `showNoteInNotification does nothing when the note is not found`() =
    runTest {
      ...
      verify(exactly = 0) { notifier.showNoteNotification(any()) }
    }
}
