...
class NoteEditViewModelLoadTest : NoteEditViewModelTestSupport() {
  ...
  fun `initializes default font size and style when remembering is disabled`() {
    every { prefs.isNoteFontSizeRememberingEnabled } returns false
    every { prefs.isNoteFontStyleRememberingEnabled } returns false
  ...
  fun `initializes remembered font size and style from prefs when remembering is enabled`() {
    every { prefs.isNoteFontSizeRememberingEnabled } returns true
    every { prefs.isNoteFontStyleRememberingEnabled } returns true
    every { prefs.lastNoteFontSize } returns 22
    every { prefs.lastNoteFontStyle } returns 3
    every { prefs.lastNoteTitleFontSize } returns 30
    every { prefs.lastNoteTitleFontStyle } returns 4
