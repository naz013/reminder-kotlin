...
class PreviewGoogleTaskViewModelTest : BaseTest() {
  ...
  override fun setUp() {
    ...
    every { uiGoogleTaskPreviewAdapter.convert(any(), any()) } returns uiPreview()
  ...
  fun `falls back to the default task list when the task's list cannot be found`() =
    runTest {
      ...
      every { uiGoogleTaskPreviewAdapter.convert(any(), defaultList) } returns uiPreview()

      viewModel.state.first()

      coVerify(exactly = 1) { uiGoogleTaskPreviewAdapter.convert(any(), defaultList) }
    }
  ...
  fun `onDeleteConfirmed shows an error when the task is not found`() =
    runTest {
      ...
      assertEquals(PreviewGoogleTaskViewModel.PreviewGoogleTaskEvent.ShowError("Failed"), event?.getContentIfNotHandled())
    }
