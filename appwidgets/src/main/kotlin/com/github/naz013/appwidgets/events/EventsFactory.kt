...
internal class EventsFactory(
  ...
  override fun onDataSetChanged() {
    ...
    invokeSuspend { getActiveRemindersUseCase() }
    ...
    when (val item = data[i]) {
      is UiBirthdayWidgetList -> {
        ...
        val data = WidgetIntentProtocol(
          mapOf<String, Any?>(
            Pair(IntentKeys.INTENT_ID, item.uuId)
          )
        )
      ...
      is UiReminderWidgetShopList -> {
        ...
        val fillInIntent = Intent()
        fillInIntent.putExtra(AppWidgetActionActivity.DATA, data)
        fillInIntent.putExtra(AppWidgetActionActivity.DIRECTION, Direction.REMINDER_PREVIEW)
